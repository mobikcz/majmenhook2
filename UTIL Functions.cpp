...
bool GameUtils::IsRifle(void* weapon)
{
	...
	CBaseCombatWeapon *pWeapon = (CBaseCombatWeapon*)weapon;
	int id = *pWeapon->m_AttributeManager()->m_Item()->ItemDefinitionIndex();
	static const std::vector<int> v = { WEAPON_GALILAR, WEAPON_AK47, WEAPON_SSG08, WEAPON_AWP, WEAPON_AUG, WEAPON_FAMAS, WEAPON_M4A1, WEAPON_M4A1_SILENCER, WEAPON_SSG08, WEAPON_AUG, WEAPON_AWP, WEAPON_SCAR20 };
	return (std::find(v.begin(), v.end(), id) != v.end());
}
