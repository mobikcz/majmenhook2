...
void CGUI::Update()
{
	...
	{
		if (!window->m_bIsOpen) {
			MenuAlpha = max(MenuAlpha - 4, 0);
			MenuTopDown = max(MenuAlpha - 1, 0);
		}
		else {
			MenuAlpha = min(MenuAlpha + 7, 255);
			MenuTopDown = min(MenuAlpha + 1, 40);
...
bool CGUI::DrawWindow(CWindow* window)
{
	...
	Render::Clear(window->m_x + 8 - 0, window->m_y + 1 + 28 * 2, window->m_iWidth - 4 - 12 + 0, window->m_iHeight - 2 - 8 - 26, Color(28, 28, 28, MenuAlpha));
	Render::Clear(window->m_x + 8 - 0, window->m_y + 1 + 28 * 2 - 6, window->m_iWidth - 4 - 12 + 0, 6, Color(40, 40, 40, MenuAlpha));
	Render::Clear(window->m_x + 8 - 0, window->m_y + 1 + 28 * 2 + window->m_iHeight - 2 - 8 - 26, window->m_iWidth - 4 - 12 + 0, 6, Color(40, 40, 40, MenuAlpha));
	Render::Clear(window->m_x + 8 - 0 - 6, window->m_y + 1 + 28 * 2 - 6, 6, window->m_iHeight - 2 - 8 - 26 + 12, Color(40, 40, 40, MenuAlpha));
	Render::Clear(window->m_x + 8 + window->m_iWidth - 4 - 12, window->m_y + 1 + 28 * 2 - 6, 6, window->m_iHeight - 2 - 8 - 26 + 12, Color(40, 40, 40, MenuAlpha));

	//Render::Clear(window->m_x, window->m_y + 58, 2, window->m_iHeight - 2 - 8 - 26, Color(255, 0, 0, MenuAlpha));
	...
	//Skeet Bar
	Render::GradientSideways(window->m_x + 9 , window->m_y + 58, window->m_iWidth / 2 + 0, 1, Color(255, 0, 0, MenuAlpha), Color(255, 0, 0, MenuAlpha), 1);
	Render::GradientSideways((window->m_x + window->m_iWidth / 2) + 10, window->m_y + 58, window->m_iWidth / 2 - 20, 1, Color(255, 0, 0, MenuAlpha), Color(255, 0, 0, MenuAlpha), 1);

	//Tab
	Render::Clear(0, 0, x, MenuTopDown, Color(50, 50, 50, 255));
	int TabCount = 6;
	if (TabCount) // If there are some tabs
	{
		int TabSize = (x) / TabCount;
		...
		{
			RECT TabArea = { 0 + (i*TabSize), 0, TabSize, MenuTopDown };
			CTab *tab = window->Tabs[i];
			if (window->SelectedTab == tab)
			{
				Render::Clear(0 + (i*TabSize), 0, TabSize, MenuTopDown, Color(255, 0, 0, 255));
			}
			else if (IsMouseInRegion(TabArea))
			{
				Render::Clear(0 + (i*TabSize), 0, TabSize, MenuTopDown, Color(255, 30, 122, 255));
			}
			RECT TextSize = Render::GetTextSize(Render::Fonts::MenuBold, tab->Title.c_str());
			Render::Text(TabArea.left + (TabSize / 2) - (TextSize.right / 2) - 30, TabArea.top + 8, Color(255, 255, 255, 255), Render::Fonts::MenuTabs, tab->Title.c_str());
			Render::Clear(0, MenuTopDown, 2000, 3, Color(255, 0, 0, 255));
		}
	}
...
void CGUI::SaveWindowState(CWindow* window, std::string Filename)
{
	...
	tinyxml2::XMLElement *Root = Doc.NewElement("majmen");
	...
	{
		MessageBox(NULL, "Failed To Save Config File!", "majmen", MB_OK);
	}
}
